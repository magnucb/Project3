...
int main(int numArguments, char **arguments)
{
    ...
    */
    ...
    if (string(integrationmethod) == string("euler")){
    ...
    else if (string(integrationmethod) == string("perihelion")){
        /* Perform verlet method on Sun-Mercury system, looking for the perihelion position
        ...
        //selected file for perihelion posiitons only
        std::ofstream perihelion_file;
        string perihelion_filename = "perihelion_positions.xyz"; //filename for perihelion positions
        string data_filename = "positions_data_"+string(bodies_string)+".xyz"; //filename for all positions
        perihelion_file.open(perihelion_filename.c_str(), ofstream::out);

        prev_r = mercury.position.length(); //establish first radial length
        //perform one step of integration
        integrator.integrateOneStep(solarSystem);
        //solarSystem.writeToFile(data_filename);
        r = mercury.position.length(); //establish second radial length

        //perform next steps of integration
        for(int timestep=1; timestep<numTimesteps; timestep++) {
            vec3 pos_cur = mercury.position;
            integrator.integrateOneStep(solarSystem);
            //solarSystem.writeToFile(data_filename);
            next_r = mercury.position.length(); //establish third radial length
            if (bodies_string == string("SunMercury")){
                if (r < prev_r){ //compare radial length to previous value
                    if (r < next_r) { //compare radial length to next value
                        perihelion_file << mercury.name << " " << pos_cur.x() << " " << pos_cur.y() << " " << pos_cur.z() << "\n";
                    }
                }
            }
            prev_r = r;
